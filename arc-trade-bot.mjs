 ...
 * Env (Railway → Variables or local .env):
 *   DISCORD_TOKEN=...
 *   APP_ID=...
 *   GUILD_ID=...
 *   DB_PATH=/data/trades.db   (or just trades.db if no volume)
 *   COOLDOWN_SECONDS=45       (optional)
 */
...
// DB path + ensure directory exists
...
client.once('clientReady', async () => {
  ...
  setInterval(() => purgeExpired.run(Date.now()), 5 * 60 * 1000) // periodic cleanup
})
  ...
  try {
    if (interaction.isChatInputCommand()) {
      ...
      if (['post', 'close'].includes(sub) && onCooldown(interaction.user.id)) {
        return interaction.reply({ flags: MessageFlags.Ephemeral, content: `Slow down a bit — you can use this again in ~${COOLDOWN_SECONDS}s.` })
      }
      ...
      if (!trade) return interaction.reply({ flags: MessageFlags.Ephemeral, content: 'Trade not found (maybe closed/expired).' })
      ...
      if (action === 'contact') {
        try {
          ...
          return interaction.reply({ flags: MessageFlags.Ephemeral, content: 'Opened a DM with the seller. Trade safely!' })
        } catch {
          return interaction.reply({ flags: MessageFlags.Ephemeral, content: 'Could not open DMs. The user may have DMs disabled.' })
        }
      }
      ...
      if (action === 'marktraded') {
        if (interaction.user.id !== trade.user_id && !interaction.memberPermissions?.has(PermissionFlagsBits.ManageMessages)) {
          return interaction.reply({ flags: MessageFlags.Ephemeral, content: 'Only the seller or a mod can mark this traded.' })
        }
      ...
      if (action === 'close') {
        if (interaction.user.id !== trade.user_id && !interaction.memberPermissions?.has(PermissionFlagsBits.ManageMessages)) {
          return interaction.reply({ flags: MessageFlags.Ephemeral, content: 'Only the seller or a mod can close this.' })
        }
    ...
    if (interaction.isRepliable()) {
      return interaction.reply({ flags: MessageFlags.Ephemeral, content: 'Something went wrong. Try again in a moment.' }).catch(() => {})
    }
  }
})
...
async function handleSearch(interaction) {
  ...
  if (!results.length) return interaction.reply({ flags: MessageFlags.Ephemeral, content: 'No active trades matched your search.' })

  const lines  = results.map(t => `#${t.id} • **Have:** ${boldCut(t.have)} → **Want:** ${boldCut(t.want)} ${t.price ? `• ${t.price}` : ''}`)
  const chunks = chunkLines(lines, 10)
  await interaction.reply({ flags: MessageFlags.Ephemeral, content: `Found **${results.length}** matches:\n\n${chunks[0]}` })
  for (let i = 1; i < chunks.length; i++) {
    await interaction.followUp({ flags: MessageFlags.Ephemeral, content: chunks[i] })
  }
}
...
async function handleList(interaction) {
  ...
  const rows = mine ? listByUser.all(user.id, 20) : listActive.all(20, 0)
  if (!rows.length) {
    return interaction.reply({ flags: MessageFlags.Ephemeral, content: mine ? 'You have no active trades.' : 'No active trades yet.' })
  }

  const lines  = rows.map(t => `#${t.id} • <@${t.user_id}> • **Have:** ${boldCut(t.have)} → **Want:** ${boldCut(t.want)} ${t.price ? `• ${t.price}` : ''}`)
  const chunks = chunkLines(lines, 10)
  await interaction.reply({ flags: MessageFlags.Ephemeral, content: chunks[0] })
  for (let i = 1; i < chunks.length; i++) {
    await interaction.followUp({ flags: MessageFlags.Ephemeral, content: chunks[i] })
  }
}
...
async function handleClose(interaction) {
  ...
  const trade = findById.get(id)
  if (!trade) return interaction.reply({ flags: MessageFlags.Ephemeral, content: 'Trade not found.' })
  if (interaction.user.id !== trade.user_id && !interaction.memberPermissions?.has(PermissionFlagsBits.ManageMessages)) {
    return interaction.reply({ flags: MessageFlags.Ephemeral, content: 'Only the seller or a mod can close this.' })
  }
  markStatus.run('closed', id)
  await interaction.reply({ flags: MessageFlags.Ephemeral, content: `Closed trade #${id}.` })
}
...
// ---------------------- Notes ----------------------
// • Invite with scopes: bot, applications.commands; perms: Send Messages, Embed Links, Read Message History (mods: Manage Messages).
// • Create #trade-market channel for /trade posts.
// • Extend later: reputation, taxonomy, auto-threads, attachments, escrow-assist.
